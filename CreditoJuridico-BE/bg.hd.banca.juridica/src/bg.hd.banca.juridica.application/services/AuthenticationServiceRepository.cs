...
using bg.hd.banca.juridica.domain.entities.security;
    ...
    public class AuthenticationServiceRepository: IAuthenticationServiceRepository
    {
        ...
        public async Task<string> GetAccessToken()
        {
            ...
            if (_authenticationMemory == null || (_authenticationMemory != null && _authenticationMemory.expired_token != null && _authenticationMemory.expired_token <= DateTime.Now))
            {
                ...
                string uri = string.Format("{0}{1}/oauth2/token", _configuration["AzureAd:Instance"], _configuration["AzureAdTenantId"]);                
                var data = new[]
                {
                new KeyValuePair<string, string>("grant_type", _configuration["AzureAd:GrantType"]),
                new KeyValuePair<string, string>("client_id", _configuration["AzureAdClientId"]),
                new KeyValuePair<string, string>("client_secret", _configuration["AzureAdClientSecret"] ),//_configuration["AzureAdClientSecret"]  // "w1n8Q~r_G3QHLeVJCYyclXy8qglyELSaCZ1t.bRR" ),
                new KeyValuePair<string, string>("resource", "api://"+_configuration["AzureAd:Audience"]) //AzureAdClientSecret
                };
